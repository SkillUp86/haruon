...
public class WebSocketMessageHandler extends TextWebSocketHandler {
    ...
    Map<String, ConcurrentLinkedDeque<WebSocketSession>> roomSessionMap = new ConcurrentHashMap<>();
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        //log.debug("쿼리파라미터(roomId) 결과 = {}", searchRoomNoInURI(session));
        // sessionMap<roomId, session> 설정
        String roomId = searchRoomNoInURI(session);
        log.debug("roomId = {}",roomId);

        // 페이로드 전송을 위한 List
        ConcurrentLinkedDeque<WebSocketSession> sessionList = new ConcurrentLinkedDeque<>();
    ...
    private String searchRoomNoInURI(WebSocketSession session) {
        UriComponents uriComponents = UriComponentsBuilder
    ...
    // 현재 대상 chatRoom에 세션이 부여되어 있는지 확인
    private boolean checkRoomNoExist(Map<String, ConcurrentLinkedDeque<WebSocketSession>> roomSessionMap, String roomId) {
        return (roomSessionMap.get(roomId) != null)? true : false;
    }
}
