...
<body class="layout-boxed">
   ...
   <!-- 모달 메일 발송 -->
	...
	<!-- 상세 클릭 시 읽음 업데이트,,,,,
	<script>
		$('#msgReader').on('click', '.mail-item-heading', function() {
			
			$.ajax({
				url: "/MsgReaders/" + "${msgNo}",
		        method: 'POST',
			}).done(function() {
		 	}).fail(function() {
	...
	<script>
		...
		document.addEventListener('click', function(event) {
	        if (event.target && event.target.id.startsWith('swichState')) {
	           console.log('swichState 버튼 클릭' + event.target.id);
	           let targetId = event.target.id.replace('swichState','');
    	...
    	$.ajax({
            ...
            $(result).each(function(index, item) {
                msgReadersHTML += `<div class="animated animatedFadeInUp fadeInUp" id="`+ "outNo" + no +`">
                                    <button type="button" id="swichState` + item.msgNo + `" class="btn">
                						<div class="mb-0">
